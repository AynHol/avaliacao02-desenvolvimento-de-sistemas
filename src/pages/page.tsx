...
export default function MarketList() {
    ...
    async function loadItens() {
        try {
            setIsLoading(true);
            const response = await axios.get("http://localhost:5505/task");
    ...
    async function handleAddItem() {
        try {
            ...
            await axios.post("http://localhost:5505/task", newTask);
            loadItens();
        } catch (error) {
    ...
    async function handleRemoveItem(id: string) {
        try {
            setIsLoading(true);
            await axios.delete(`http://localhost:5505/task/${id}`);
            loadItens();
        } catch (error) {
    ...
    async function handleCheckTask(id: string) {
        try {
            setIsLoading(true);
            await axios.patch(`http://localhost:5505/task/${id}/completed`);
            loadItens();
        } catch (error) {
    ...
    return (
        <div className={styles.container}>
            ...
            <div className={styles.groupInput}>
                <input type="text" placeholder="Adicionar uma nova tarefa" className={styles.inputTask} value={newTaskText} onChange={(e) => setNewTaskText(e.target.value)} />
